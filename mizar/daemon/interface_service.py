...
print("current interface : "+str(ifaces[0]))
...
class InterfaceServer(InterfaceServiceServicer):
    ...
    def __init__(self):
        ...
        cmd = 'ip addr show %s | grep "inet\\b" | awk \'{print $2}\' | cut -d/ -f1' % ifaces[0]
        r = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE)
        self.droplet_ip = r.stdout.read().decode().strip()

        cmd = 'ip addr show %s | grep "link/ether\\b" | awk \'{print $2}\' | cut -d/ -f1' % ifaces[0]
        r = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE)
